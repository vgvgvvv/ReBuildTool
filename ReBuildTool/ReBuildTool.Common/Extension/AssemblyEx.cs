...
namespace ReBuildTool.Common
{
    public static class AssemblyEx
    {
        ...
        public static bool LoadAssemblyFromFileSafely(string assemblyPath, out Assembly assembly)
        {
            assembly = null;
            ...
            if (!File.Exists(assemblyPath))
            {
                ...
                var fromFramework = Path.Combine(assemblyRoot, assemblyPath);
                if (!File.Exists(fromFramework))
                {
                    Log.Error($"File.Exists({assemblyPath})");
                    return false;
                }
                assemblyPath = fromFramework;
            }

            if (AssemblyLoadContext.Default.Assemblies
                .Any(ass => ass.GetName().Name == Path.GetFileNameWithoutExtension(assemblyPath)))
            {
                Log.Error($"Repeat assembly {assemblyPath}");
                return false;
            }

            assembly = Assembly.LoadFrom(assemblyPath);
            AssemblyLoadContext.Default.LoadFromAssemblyName(assembly.GetName());
